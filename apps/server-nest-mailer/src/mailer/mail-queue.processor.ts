...
export class MailProcessor {
  ...
  async sendConfirmationToken(job: Job<{ user: any; token: string }>) {
    try {
      ...
      await this.redisService
        .getClient()
        .set(`confirm-account:${token}`, user.id, 'EX', 1000 * 60 * 60 * 1); // 1 hour until expires

      const email = await this.mailerService.sendMail({
        to: user.email,
        subject: '이메일 확인을 완료해주세요.',
  ...
  async sendResetToken(job: Job<{ user: any; token: string }>) {
    try {
      ...
      await this.redisService
        .getClient()
        .set(`reset-password:${token}`, user.id, 'EX', 1000 * 60 * 60 * 1); // 1 hour until expires
